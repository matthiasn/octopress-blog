...
* Immutable data structures are very powerful and make reasoning about an application much more straightforward. Implementing an **undo** functionality becomes almost trivial with this approach, as I will try to show you in this article.

Here is the new client in action. Note that **undo** will revert the application state by one step (including name changes and such), whereas **undo all** will very quickly go through all steps until the beginning of time.
...
Let us go through this file step by step:
...
+ *SseChat.setProps* is the function that passes props to the top level component. Once the JSX is compiled and initialized, this replaces the placeholder function inside react-interop.js.

+ At the end of the file, *ScalaApp.triggerReact* is called. This is only so that the initial state (with a random name) is rendered, independent of a message from the server being received. It just makes the initial rendering a bit smoother, other than that it is superfluous.
