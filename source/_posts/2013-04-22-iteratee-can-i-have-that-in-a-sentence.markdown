...
We have a systemwide EventBus to which we attach for every user connection an actor listening to Tweets and pushing them into the tweetChannel. Somehow, and we will get there, this Tweet will reach the tweetlistIteratee, where it will be prepended to the accumulator. Then the tweetlistInterceptor function will be executed, doing our computations and pushing the result into the WebSocket connection to the client.

So far so good. But we know that the Iteratee is an immutable data type:
...
This is what confused me initially. How does the enumerator keep track of the current Iteratee to feed with the next element in the stream? Does this mean the enumerator has mutable internal state, holding the latest Iteratee? Turns out it depends on the definition of mutable state. Some enumerators such as 

An Iteratee can be in one of three states, Cont, Done and Error. The Done state does not seem particularly useful when dealing with truly open-ended streams. When is a Twitter stream done? When Twitter ceases to exist? For this reason I also do not care about any result on a Done state. The results over a stream are only meaningful incrementally. But a Done state could certainly be useful in other applications, such as when reading from a large file in smaller chunks or when streaming data from ReactiveMongo until the iterator runs out of data. Then a final computation makes sense.
